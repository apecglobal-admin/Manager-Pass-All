...
const regex = /if \(canViewUrl\) return 'Chưa có URL';\s*closeItemMenus\(\);\s*if \(willOpen\) wrap\?\.classList\.add\('menu-open'\);\s*\}\)\);/g;

const newFunc = `if (canViewUrl) return 'Chưa có URL';
  return 'Bị giới hạn';
}
...
console.log('Fixed mangle correctly');
